...
- **Legacy accounts** — users created before password hashing still carry a plaintext `password` (those values were once in git history). While `SMTP_*` is unset they log in normally and are upgraded to bcrypt on the spot; once SMTP is configured, login refuses them with `PASSWORD_RESET_REQUIRED` and sends them through "Forgot Password" so the exposed password is never accepted again.
