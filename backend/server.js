...
const razorpay = new Razorpay({
    key_id: process.env.RAZORPAY_KEY_ID || '***REMOVED***',
    key_secret: process.env.RAZORPAY_KEY_SECRET || '***REMOVED***'
});
...
app.post('/api/verify-payment', (req, res) => {
    try {
        ...
        const hmac = crypto.createHmac('sha256', process.env.RAZORPAY_KEY_SECRET || '***REMOVED***');
